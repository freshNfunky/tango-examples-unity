...
public class MeshBuilderWithPhysicsGUIController : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(10, 80, 140, 80), "<size=30>Clear</size>"))
        {
            m_dynamicMesh.Clear();
        ...
        if (GUI.Button(new Rect(10, 180, 140, 80), "<size=30>" + text + "</size>"))
        {
            m_isEnabled = !m_isEnabled;
            m_tangoApplication.Set3DReconstructionEnabled(m_isEnabled);
        }

        if (GUI.Button(new Rect(10, 280, 140, 80), "<size=30>Export</size>"))
        {
